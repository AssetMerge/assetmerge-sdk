...
export const createAddEtherLiquidityParams = (nftToken: ERC721, to: string, ftReserves: BigNumber, globalNftWeight: BigNumber, inputs: { item: ERC721Item, price: BigNumber }[], customFloor?: BigNumber): [string, BigNumber[], BigNumber[], string, { value: BigNumber }] => {
  const defaultWeight = BigNumber.from(10).pow(18);
  inputs.sort((a, b) => {
    if (a.price.lt(b.price)) return -1
    else if (a.price.gt(b.price)) return 1
